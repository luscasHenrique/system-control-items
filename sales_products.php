...
// Verifica se o usuário tem a role necessária (Admin, SuperAdmin, Seller)
if (!in_array($_SESSION['role'], ['Admin', 'SuperAdmin', 'Seller'])) {
    header("Location: unauthorized.php"); // Redireciona para página de não autorizado
    exit();
}
...
// Contar o número total de registros para "Luna Editora"
$totalStmt = $conn->prepare("SELECT COUNT(*) as total FROM stock_logs l
...
// Buscar os registros de logs da "Luna Editora" com paginação
$stmt = $conn->prepare("
    SELECT 
        l.id, 
        l.product_id, 
...
<head>
    ...
    <title>Registro de Alterações de Produtos - Luna Editora</title>
...
<body class="bg-gray-100 min-h-screen">
    ...
    <div class="max-w-7xl mx-auto mt-10 bg-white p-6 rounded-lg shadow-md">
        <h1 class="text-2xl font-bold mb-6 text-blue-700 text-center">Registro de Alterações de Produtos - Luna Editora</h1>
            ...
            <table id="logTable" class="w-full border-collapse border border-gray-300 text-center">
                <thead class="bg-gray-200">
                    <tr>
                        ...
                        <th class="border border-gray-300 p-2">Nº da Atualização</th>
